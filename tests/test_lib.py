...
@pytest.fixture
def desc(old: JSONDict, result: JSONDict, guru: Metaguru) -> str:
    ...
    if "/album/" in result["data_url"]:
        ...
        result.update(albumartist=result.pop("artist", ""), tracks=get_tracks(result))
        artist, title = result.get("albumartist", ""), result.get("album", "")
    else:
