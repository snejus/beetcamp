...
ResultType = Literal["album", "artist", "fan", "label", "track"]
...
def _api_result_to_release(result: BandcampSearchResult) -> SearchResult:
    ...
    if result_type == "label" and result.get("is_label") is False:
        result_type = "artist"
    return {
        "type": result_type,
...
def search_json(query: str, search_type: SearchTypeCode | None) -> list[SearchResult]:
    payload: BandcampSearchRequest = {
        "search_text": query,
        "search_filter": SEARCH_TYPE_TO_FILTER.get(search_type or "", ""),
        "full_page": True,
    }
